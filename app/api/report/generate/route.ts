...
import { generateReportFromPreview } from '../../../../lib/llm/report';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const out = await generateReportFromPreview({ date, preview });
    return NextResponse.json(out, { status: 200 });
  } catch (e: any) {
    const reqId = e?.requestId || e?._request_id || e?.headers?.get?.('x-request-id');
    console.error('report_generate_error', { message: e?.message, requestId: reqId });
    if (e?.statusCode === 422) {
